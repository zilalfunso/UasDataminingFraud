...
# Info tim penyusun (dari cover Laporan Proyek UAS)
TIM = {
    "mata_kuliah": "Data Mining",
    "dosen": "Fathoni Mahardika, S.Kom., M.T.",
    "kelompok": "Kelompok 2",
    "universitas": "Universitas Sebelas April (UNSAP)",
    "anggota": [
        ("Ahmad Al Fauzi", "230660121014"),
        ("Diki Muhammad Alfikri", "230660121111"),
        ("Ridho Makwa Nugraha", "230660121083"),
        ("Shidiq Septiana", "230660121018"),
    ],
}
...
with st.sidebar:
    ...
    st.markdown(f"**{TIM['kelompok']}** — {TIM['universitas']}")
    for nama, nim in TIM["anggota"]:
        st.markdown(f"- {nama} — `{nim}`")
    st.caption(f"Mata Kuliah: {TIM['mata_kuliah']} | Dosen Pengampu: {TIM['dosen']}")
...
with tabs[0]:
    ...
    with c1:
        st.markdown(f"**{TIM['kelompok']}**")
        st.markdown(TIM["universitas"])
        st.caption(f"Mata Kuliah: {TIM['mata_kuliah']}")
        st.caption(f"Dosen Pengampu: {TIM['dosen']}")
    with c2:
        st.dataframe(
            pd.DataFrame(TIM["anggota"], columns=["Nama", "NIM"]),
...
st.markdown("---")
anggota_str = ", ".join(nama for nama, _ in TIM["anggota"])
st.caption(
    f"Dashboard dibuat oleh **{TIM['kelompok']}** ({anggota_str}) — "
    f"Mata Kuliah {TIM['mata_kuliah']} | Dosen Pengampu: {TIM['dosen']} | {TIM['universitas']}"
)
